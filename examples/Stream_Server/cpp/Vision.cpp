#ifndef USER_RELEASE // uncomment for release mode
// #define USER_RELEASE
#endif
...
class OutputLayer : public CJ::Layer {
 public:
	...
	void onAttach() override {
		...
		// Stream Server
		#ifndef RELEASE
		_streamServer.start(); // start the stream server
		#endif // RELEASE
	...
	void onUpdate() override {
		...
		#ifndef RELEASE // Don't run on release mode. Only use fast point to point nt above (fastest)

		#ifndef COPROC // run on local machine
		...
		#ifdef COPROC // run on coprocessor
		_streamServer.output(_images.processed);
		#endif // COPROC
 ...
 private:
	...
	// Stream Server (High latency), however should not affect the speed of the main thread too much
	CJ::Output::Stream _streamServer{5802}; // port number 5802
};
