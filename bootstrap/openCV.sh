...
sudo mkdir release
cd release
sudo cmake -D HAVE_VIDEOINPUT=ON -D HAVE_DSHOW=ON -D BUILD_TIFF=ON -D OPENCV_GENERATE_PKGCONFIG=ON -D WITH_CUDA=ON -D ENABLE_AVX=OFF -D WITH_OPENGL=OFF -D WITH_OPENCL=OFF -D WITH_IPP=OFF -D WITH_TBB=ON -D BUILD_TBB=ON -D WITH_EIGEN=OFF -D WITH_V4L=ON -D WITH_MSMF=ON -D WITH_GSTREAMER=ON -D WITH_FFMPEG=ON -D WITH_VTK=OFF -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/usr/local -D OPENCV_EXTRA_MODULES_PATH=/opt/opencv_contrib/modules /opt/opencv/
sudo make -j4
sudo make install
sudo ldconfig
